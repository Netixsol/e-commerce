...
export default function Checkout() {
  ...
  const updatedQuantity = (id,qty) => {
    // filter product with id in all product
     const product = allProducts.find(product => product._id === id.product._id);
  ...
  useEffect(() => {
    ...
    const options = {
      ...
      const removeProduct = (id) => {
        deleteCart(id,user_id).then((res) => {
          setCartProducts(res.data.products)
        }
        ).catch((err) => {
          console.log("Error",err);
                                ...
                                <div className="ml-4 flex flex-1 flex-col">
                                  <div>
                                    <div className="flex justify-between text-base font-medium text-gray-900">
                                      ...
                                      <XIcon  onClick={() => removeProduct(product._id)}className="block h-6 w-6 hover:text-red-500 cursor-pointer" aria-hidden="true" />
                                    </div>
